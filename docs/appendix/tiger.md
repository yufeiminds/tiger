...
空白（空格，制表符，换行，回车，换页）或者注释可以出现在标识符之间，并被忽略。一段注释以 `/*` 开始并以 `*/` 结尾。注释可以嵌套。
...
一个左值表示一块可被赋值的存储区域：变量、参数、Record 的字段和 Array 的元素。一个大小为 n 的数组元素索引为 0，1，… ，n - 1。

### 2.2 返回值

过程调用，赋值，`if-then`，`while`，`break` 以及 `if-then-else` 返回无，并且没有像所期望的那样返回值（比如，`(a := b) + c` 是不合法的）。
...
表达式 *type-id* { $$field-list_{opt}$$ } （允许零个或更多字段）创建一个类型为 *type-id* 的新 Record 实例。字段名，表达式类型和它们的顺序必须明确地与给定的 Record 类型匹配。

表达式 *type-id* [ *expr* ] **of** *expr* 创建一个类型为 *type-id* ，大小为括号里给定表达式大小的新 Array。初始化时，该 Array 被 **of** 后给定表达式的值填充。上述两个表达式按照它们出现的顺序被求值。
...
二元操作符 `>` ，`<` ，`>=` ，`<=` 比较它们的操作数，而且他们或者都是整型，或者都是字符串，如果比较结果正确就返回整数1，否则返回0。字符串的比较结果取决于标准ASCII顺序。

二元操作符 `=` 和 `<>` 可以比较任意两个相同类型的操作数，并且返回整数0或1。整数取决于他们是否有相同的值。字符串取决于它们包含的字符是否相同。两个类型是 Record 的对象取决于它们是否是同一个 Record 的引用。
...
Array 和 Record 赋值是通过引用而非值。赋值一个数组或者记录到一个变量时，会创建一个绑定。这意味着稍后对于这个变量或者值的改变会给所有地方带来影响。将一个数组或者记录作为函数实参来传递，其行为也是类似的。
...
有三种 **type** 的引用形式（在声明中创建一个别名），带有名字的 Record，有类型的字段（像是一个C的结构体，不同的 Record 会重用这些字段名），和一个数组。
...
第一种形式是一个过程定义，第二个是一个函数定义。函数返回一个特定类型的值；过程仅仅为了它们的作用而被调用。所有的形式都允许指定由零个或多个带有类型的参数组成的一个列表，它们是按值传递的。参数的作用域在 *expr* 中。
